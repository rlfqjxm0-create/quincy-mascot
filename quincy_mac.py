"""퀸시 마스코트 — macOS 앱 진입점 (.app 패키징용).

윈도우판(quincy_app.py)과 같은 구조지만 맥 경로 규칙을 따른다:
  설정·타이머 기록  ~/Library/Application Support/QuincyMascot/
  자동 업데이트 파일 ~/Library/Application Support/QuincyMascot/live/
...
REPO = "rlfqjxm0-create/quincy-mascot"   # 공개 배포 전용 레포
BRANCH = "main"
TIMEOUT = 20
RETRY = 3
...
APPDIR = os.path.expanduser("~/Library/Application Support/QuincyMascot")
...
def _fetch(path, timeout=None, tries=None):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "QuincyMascot-updater"})
...
def main():
    ...
    live_parts = os.path.join(LIVE, "parts_quincy")
    ...
    if mascot is None:
        import mascot
        char_dir = os.path.join(BUNDLE, "parts_quincy")
