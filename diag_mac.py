...
def dump_windows(tag, prev):
    ...
    try:
        ...
        for key, v in sorted(cur.items()):
            ...
            if old != v or grow:
                print("[창:%s] %-28s %4dx%-4d at(%d,%d) 보임=%s 층=%s%s"
                      % (tag, key, v[0], v[1], v[2], v[3], v[4], v[5], grow),
                      flush=True)
        return cur
...
# ── 음악 연출 (요청: 맥에서 음표·리듬이 안 나온다?) — 재생기 없이 '재생 중'
#    상태만 흉내 내 음표가 뜨는지, 리듬 타기가 도는지 숫자와 그림으로 남긴다
if "--notes" in sys.argv:
    try:
        print("[음표] fx_imgs=%s 매끈=%s fx_soft=%s 자는중=%s"
              % (sorted(m.fx_imgs), getattr(m, "_smooth_on", "?"),
                 getattr(m, "_fx_soft", "?"), getattr(m, "_sleeping", "?")),
              flush=True)
        m.us["show_timer"] = True
        m.timer_on = True
        m._yt = {"ready": True, "playing": True}
        m._yt_want = True
        n_img0 = len([i for i in m.canvas.find_all() if m.canvas.type(i) == "image"])
        seen_max = 0
        for _i in range(90):
            now = time.time()
            m._yt_notes(now)
            m.root.update()
            seen_max = max(seen_max, len(m.notes))
            time.sleep(0.03)
        n_img1 = len([i for i in m.canvas.find_all() if m.canvas.type(i) == "image"])
        print("[음표] 3초 동안 떠 있던 최대 음표 수=%d · 지금=%d · 캔버스 그림 %d→%d · _yt_note=%s"
              % (seen_max, len(m.notes), n_img0, n_img1, m._yt_note is not None), flush=True)
        print("[음표] _fail=%s" % ({k: v for k, v in m._fail.items() if v},), flush=True)
        grab_window(m.root, "notes")
        # 리듬 타기 — 음악 중 15분마다 도는 몸짓을 지금 바로 시작해 본다
        m._gest_start("groove")
        for _i in range(45):
            m._yt_notes(time.time())
            m.root.update()
            time.sleep(0.03)
        print("[음표] groove gest=%s tilt=%.1f hands=%s" % (
            getattr(m, "gest", None) and getattr(m, "gest", None)[0]
            if isinstance(getattr(m, "gest", None), (list, tuple)) else getattr(m, "gest", None),
            float(getattr(m, "_g_tilt", 0.0)), bool(getattr(m, "_g_hands", None))), flush=True)
        grab_window(m.root, "groove")
        # 플레이리스트 창을 열어 Tk '항상 위' 창의 층(level)을 남긴다 —
        # 영상 창(VID_LEVEL)이 그 위에 오는지 판단하는 숫자 (퀸시 제보)
        try:
            m._yt_avail = True
            m._bgm_win("pl")
            m.root.update()
            dump_windows("플리열림", wins)
        except Exception as e9:
            print("[음표] 플리 창 열기 실패 %r" % e9, flush=True)
        try:
            with open(os.path.join(m.state_dir, ".error.log"), encoding="utf-8") as fp:
                print("[음표] error.log: " + fp.read()[-1500:].replace(chr(10), " | "), flush=True)
        except Exception:
            print("[음표] error.log 없음", flush=True)
    except Exception as e:
        import traceback
        print("[음표] 단계 실패 %r %s" % (e, traceback.format_exc().replace(chr(10), " | ")), flush=True)
    try:
        m.close()
    except Exception:
        pass
